...
if config_env() in [:dev, :test] do
  if File.exists?(".env"), do: source!([".env"])
  if File.exists?(".env.#{config_env()}"), do: source!([".env.#{config_env()}"])
  if File.exists?(".env.local"), do: source!([".env.local"])

  config :engine, Engine.Repo,
    username: env!("DB_USER", :string, "postgres"),
    password: env!("DB_PASSWORD", :string, "postgres"),
    hostname: env!("DB_HOST", :string, "localhost"),
    database: env!("DB_NAME", :string, "shiplio_dev")
end

if config_env() == :prod, do: (if File.exists?(".env.prod"), do: source!([".env.prod"]))
...
if config_env() == :prod do
  ...
  config :engine, :dns_cluster_query, env!("DNS_CLUSTER_QUERY", :string, nil)
